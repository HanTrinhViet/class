...
public class TicketControllerTest {
    ...
    public void givenTicketNotInNewState_whenAssigningAgent_thenThrowException() throws Exception {
        ...
        when(ticketService.assignAgentToTicket(ticketId, agentId)).thenThrow(new InvalidTicketStateException(Constants.ONLY_NEW_TICKETS_CAN_BE_ASSIGNED_TO_AN_AGENT));
    ...
    @Test
    public void givenNonexistentTicket_whenAssigningAgent_thenThrowException() throws Exception {
    ...
    public void givenClosedTicket_whenResolving_thenThrowException() throws Exception {
        ...
        when(ticketService.resolveTicket(ticketId))
                .thenThrow(new InvalidTicketStateException(Constants.ONLY_TICKETS_IN_PROGRESS_CAN_BE_RESOLVED));

        mockMvc.perform(put("/tickets/{id}/resolve", ticketId)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(content().string(Constants.ONLY_TICKETS_IN_PROGRESS_CAN_BE_RESOLVED));
    }
